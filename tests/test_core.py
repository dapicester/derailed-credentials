...
def test_master_key_from_env(credentials):
...
def test_master_key_from_file(credentials, temp_dir):
...
def test_master_key_env_priority(credentials, temp_dir):
    env_key = "env-key"
    file_key = "file-key"
...
def test_master_key_missing(credentials):
    with pytest.raises(MasterKeyMissing):
        credentials.master_key


def test_encrypt_decrypt_roundtrip(credentials_with_key):
...
def test_config_empty_file(credentials_with_key):
...
@pytest.fixture
def credentials_with_data(credentials_with_key, sample_data):
    credentials_with_key.config = sample_data
    return credentials_with_key


def test_config_with_data(credentials_with_data, sample_data):
    assert credentials_with_data.config == sample_data


def test_get_simple_key(credentials_with_data):
...
def test_get_nested_key(credentials_with_data):
...
def test_show(credentials_with_data, sample_data):
    yaml_output = credentials_with_data.show()
    parsed = yaml.safe_load(yaml_output)
    assert parsed == sample_data


def test_create_master_key_file(credentials, temp_dir):
    key_path = temp_dir / "master.key"
...
def test_create_master_key_file_exists_abort(credentials, temp_dir):
    key_path = temp_dir / "master.key"
...
def test_corrupted_encrypted_file(temp_dir):
    ...
    creds = Credentials(credentials_path=str(creds_path), master_key_path=str(key_path))

    with pytest.raises(CredentialsError):
        creds.config


def test_invalid_yaml_in_decrypted_content(temp_dir):
    creds_path = temp_dir / "credentials.yml.enc"
    key_path = temp_dir / "master.key"
...
def test_permission_denied_master_key(temp_dir):
    key_path = temp_dir / "master.key"
...
def test_empty_credentials_file(temp_dir):
    creds_path = temp_dir / "credentials.yml.enc"
    key_path = temp_dir / "master.key"
...
def test_dynamic_attributes(credentials_with_data):
