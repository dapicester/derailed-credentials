...
@pytest.fixture
def cli_env(temp_dir, project_root_dir):
    # TODO: DRY
...
def test_cli_generate_key(temp_dir, project_root_dir):
...
def test_cli_generate_key_already_exist(master_key, temp_dir, project_root_dir):
    key_path = temp_dir / "master.key"
    assert key_path.exists()
...
@pytest.fixture
def credentials_with_data(request, cli_env):
...
def test_cli_edit(credentials_with_data, cli_env):
...
def test_cli_show(credentials_with_data, cli_env):
    result = run_cli(["show"], cli_env)
    assert result.exit_code == 0
    assert "test_key: test_value" in result.output


def test_cli_fetch(credentials_with_data, cli_env):
    result = run_cli(["fetch", "test_key"], cli_env)
    assert result.exit_code == 0
    assert result.output.rstrip() == "test_value"


@pytest.mark.sample_data({"some": {"nested": "value"}})
def test_cli_fetch_nested(credentials_with_data, cli_env):
    result = run_cli(["fetch", "some.nested"], cli_env)
    assert result.exit_code == 0
    assert result.output.rstrip() == "value"


@pytest.mark.sample_data({"some": {"nested": "value"}})
def test_cli_fetch_error(credentials_with_data, cli_env):
    result = run_cli(["fetch", "some.nested.deep.value"], cli_env)
    assert result.exit_code == 1
    assert "Invalid or missing credentials path", result.output


def test_cli_diff_nothing(credentials_with_data, cli_env):
    result = run_cli(["diff"], cli_env)
    assert result.exit_code == 0
...
@pytest.fixture
def gitattributes(cli_env, request):
...
@pytest.fixture
def gitattributes_enrolled(gitattributes):
    gitattributes.write_text(GITATTRIBUTES_ENTRY)
    return gitattributes


def test_cli_diff_enroll_already_enrolled(gitattributes_enrolled, cli_env):
    result = run_cli(["diff", "--enroll"], cli_env)
    assert result.exit_code == 0
    assert "Project is already enrolled in credentials file diffing" in result.output


def test_cli_diff_disenroll(gitattributes_enrolled, cli_env):
    result = run_cli(["diff", "--disenroll"], cli_env)
    assert result.exit_code == 0
    assert "Disenrolled project from credentials file diffing" in result.output


def test_cli_diff_disenroll_not_enrolled(gitattributes, cli_env):
    result = run_cli(["diff", "--disenroll"], cli_env)
    assert result.exit_code == 0
...
@pytest.fixture
def git_config(cli_env, request):
...
def test_cli_edit_configure_diff_driver(
    gitattributes_enrolled, git_config, credentials_with_data, cli_env
):
